...
#        self.test = test_gui.TestGrid(grid_size) #test purpose
    ...
    def play(self, coordinates=None):
        ...
        if not coordinates:
            ...
            defeat_in = self.set_defense_score()
            self.set_offense_score(defeat_in)
            top_score = [0]
            for column in range(0, self.grid_size):
                for row in range(0, self.grid_size):
                    x, y = column, row
...
#        self.test.show_sb()
            ...
            for direction in self.directions:
                ...
                if x < self.grid_size and y < self.grid_size:
                    if ([x, y] not in self.my_moves) and (self.within_grid([x, y])):
                        if defeat_in > 4:
                            defeat_in = 4
                        self.score_grid[x][y] += self.default_score
                        ...
                        if len(line) == 4 and line[1] in self.rival_moves:
                            if self.score_grid[x][y] < 2 * self.default_score:
                                self.score_grid[x][y] += self.default_score
                            if defeat_in > 3:
                                defeat_in = 3
                        # maybe create case for three singles situation X_X_X here
        # where the magic happens (add score to ends of rival lines):
            ...
            if len(line_extension_1) + len(line_extension_2) + len(line) >= 5:
                for line_extension in [line_extension_1, line_extension_2]:
                    ...
                    if len(line_extension_1) + len(line) < 5 or len(line_extension_2) + len(line) < 5:
                        variable_score -= self.default_score
                    if len(line_extension) + len(line) < 5:
                        variable_score -= self.default_score
                    if len(line) == 3 and len(line_extension_1) > 0 and len(line_extension_2) > 0:
                        variable_score += 3 * self.default_score
                    if len(line) + counter == 4:
                        variable_score += 5 * self.default_score
                    if len(line_extension) > 1 and line_extension[1] in self.rival_moves:
                            x, y = line_extension[0]
                            self.score_grid[x][y] += (len(line) + counter) * self.default_score
                    for x, y in line_extension:
                        self.score_grid[x][y] += self.default_score * (len(line) - distance + 1) + variable_score
            ...
            for direction in self.directions:
                for cell in line:
                    ...
                    if len(chkline) == 5 and chkline[1] not in self.rival_moves and chkline[2] in self.rival_moves:
                        x, y = chkline[1]
                        if self.score_grid[x][y] < 3 * self.default_score:
                            self.score_grid[x][y] = 3 * self.default_score
        return defeat_in
    ...
    def set_offense_score(self, defeat_in):
        ...
        for line in my_lines:
            direction_1 = self.check_direction(line[-1], line[0])
            x, y = self.cellxy(line[0], direction_1, 1)
            line_extension_1 = self.cells_on_line('empty&me', direction_1, [x, y], 5 - len(line), True)
            direction_2 = self.check_direction(line[0], line[-1])
            x, y = self.cellxy(line[-1], direction_2, 1)
            line_extension_2 = self.cells_on_line('empty&me', direction_2, [x, y], 5 - len(line), True)
            if len(line) + len(line_extension_1) + len(line_extension_2) < 5:
                continue
            for line_extension in [line_extension_1, line_extension_2]:
                if len(line_extension) == 0:
                    continue
                ...
                for cell in line_extension:
                    if cell in self.my_moves:
                        counter += 1
                win_in = 5 - len(line) - counter
                if win_in == 1:
                    x, y = line_extension[0]
                    self.score_grid[x][y] += 20 * self.default_score
                    return
                # Modify score based on count of turns needed to win and to be defeated
#                offense_score = self.default_score + (defeat_in - win_in + 1) * (self.default_score / 5)
                if defeat_in >= win_in:
                    offense_score = int(self.default_score * 1.2)
                elif defeat_in < win_in:
                    offense_score = int(self.default_score / 5)
                else:
                    offense_score = self.default_score
                variable_score = 0
                if len(line_extension_1) < 1 or len(line_extension_2) < 1:
                    variable_score -= offense_score
                if len(line_extension) + len(line) < 5:
                    variable_score -= offense_score
                if len(line) == 3 and len(line_extension_1) > 1 and len(line_extension_2) > 1:
                    variable_score += 3 * offense_score
                if len(line_extension) > 1 and line_extension[1] in self.rival_moves:
                    x, y = line_extension[0]
                    self.score_grid[x][y] += offense_score
                distance = 0
                for x, y in line_extension:
                    self.score_grid[x][y] += offense_score * (len(line) - distance + 1) + variable_score
                    if self.score_grid[x][y] < 0:
                        self.score_grid[x][y] = 0
                    distance += 1
        if defeat_in < 4:
            ...
            offense_score = int(self.default_score * 1.1)
        for cell in my_singles:
            for i in range(0, 4):
                line_extension_1 = self.cells_on_line('empty&me', self.directions[i], cell, 5, True)
                line_extension_2 = self.cells_on_line('empty&me', self.directions[i + 4], cell, 5, True)
                if len(line_extension_1) > 1 and len(line_extension_2) > 1:
                    if len(line_extension_1) + len(line_extension_2) > 5: #>5 because both extensions includes 1st cell
                        x, y = line_extension_1[1]
                        self.score_grid[x][y] += offense_score
                        x, y = line_extension_2[1]
                        self.score_grid[x][y] += offense_score
    ...
    def cells_on_line(self, option, direction, start, length, stop=False):
        ...
        if option == 'empty&me':
            return_line = []
            for cell in whole_line:
                if cell not in self.rival_moves:
                    return_line.append(cell)
                elif stop:
                    break
            return return_line
